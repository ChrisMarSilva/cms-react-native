                ...
                <View style={{ justifyContent: 'center', alignItems: 'center', width: '85%', height: '70%', borderRadius: 25, backgroundColor: '#fff', borderWidth: 0, borderColor: 'blue' }}>
                    {isLoadingRecebedor ? (
                        ...
                        <View style={{ flex: 1, width: '100%', justifyContent: 'center', alignItems: 'center', borderWidth: 0, borderColor: 'blue' }}>
                            ...
                            {parseFloat(valor) > 0 ? <Text style={{ color: '#000', fontWeight: 'bold', fontSize: 30, marginBottom: 20 }}> R$ {HelperNumero.GetMascaraValorDecimal(parseFloat(valor))} </Text> : <TextInputMask type={'money'} options={{ precision: 2, separator: ',', delimiter: '.', unit: 'R$ ', suffixUnit: '' }} value={valor} onChangeText={(value) => setValor(value)} onSubmitEditing={Keyboard.dismiss} style={{ textAlign: 'center', fontSize: 25, fontWeight: 'bold', height: 40, borderBottomWidth: 1, borderBottomColor: '#555', color: '#000', backgroundColor: '#fff', marginBottom: 30, width: '80%' }} />}
