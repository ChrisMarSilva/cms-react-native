...
import { StyleSheet, SafeAreaView, View, Text, TouchableOpacity, TextInput, Keyboard, Alert, Dimensions, Platform, } from 'react-native'
    ...
    const _onPressConfirmar = async (codigo) => {
        ...
        if (codigo != '') {
            ...
            if (mocas == null || mocas.length <= 0) {
                ...
                alert('', 'Moça não localizada!', { textConfirm: '     OK     ' });
                return false
            }
            const moca = mocas[0]
            if (moca.situacao != 'A') {
                //Alert.alert('', 'Moça inativa!')
                alert('', 'Moça inativa!', { textConfirm: '     OK     ' });
                return false
            }
