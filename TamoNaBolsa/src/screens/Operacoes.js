...
import { limpaOperacoes, modificaMsgOperacoes, buscaListaAtivosOperacoes, buscaListaOperacoes } from '../store/ducks/operacoes'
...
class Operacoes extends React.Component {
  ...
  componentDidMount() {
    ...
    const txtDataFim = moment().endOf('month')
    this._CarregarDados(txtCodAtivo, moment(txtDataIni).format("YYYYMMDD"), moment(txtDataFim).format("YYYYMMDD")) 
  ...
  componentWillUnmount() {
    this.props.limpaOperacoes()
  }

  componentDidUpdate(prevState, prevProps) {
    if (this.props.txtFiltroMsgErro != '') { 
      HelperToast.displayMsgError(this.props.txtFiltroMsgErro)
     this.props.modificaMsgOperacoes('')
    }
  }
  
  _HandleOnPressItem = (data, tipo, codigo, qtd, vlrPreco, corretora, vlrCorregatem, vlrTaxas, vlrTotal, vlrCustos, tipoInvest) => { 
    this.props.navigation.navigate('OperacoesDetalhe', {data: data, tipo: tipo, codigo: codigo, qtd: qtd, vlrPreco: vlrPreco, corretora: corretora, vlrCorregatem: vlrCorregatem, vlrTaxas: vlrTaxas, vlrTotal: vlrTotal, vlrCustos: vlrCustos, tipoInvest: tipoInvest})
  }
  ...
  _HandleOnPressFiltro = () => {
    ...
    this._CarregarDados(txtCodAtivo, txtDataIni, txtDataFim) 
  } 

  _CarregarDados = async (txtCodAtivo, txtDataIni, txtDataFim) => { 
    this.props.buscaListaOperacoes(txtCodAtivo, txtDataIni, txtDataFim) 
  }

  _CarregarListas = async () => {
    this.props.buscaListaAtivosOperacoes() 
  }
  ...
  render() {
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: '#ECF0F1', }}>
        ...
        {
          !this.props.isLoadingOperacoes && this.props.dataProvider._data.length > 0 &&
          // <FlatList
          //   style={{ marginBottom: 0, }}
          //   data={this.props.lstOperacoes}
          //   scrollEnabled={true} 
          //   keyExtractor={(item, index) => item[10].toString()}
          //   ListEmptyComponent={() => <Text style={{ flex: 1, fontSize: 18, fontWeight: 'bold', marginTop: 20, textAlign: 'center', }}>{!this.props.isLoadingOperacoes && this.props.txtFiltroMsgErro == '' ? 'Nenhum registro encontrado...' : null}</Text>}
          //   renderItem={({ item, index }) => <OperacoesListaItem index={index} item={item} _HandleOnPressItem={this._HandleOnPressItem} />}
          ...
          <RecyclerListView
            ...
            canChangeSize={true}
            rowRenderer={(type, item, index) => <OperacoesListaItem type={type} index={index} item={item} _HandleOnPressItem={this._HandleOnPressItem}/>}
...
function OperacoesListaItem(props) {
  ...
  return(
    <View key={props.item[10]} style={{ flex: 1, backgroundColor: '#fff', borderRadius: 10, elevation: 1, height: 60, marginTop: 7, marginLeft: 7, marginRight: 7, }}>
      <TouchableOpacity style={{ flex: 1, flexDirection: "row", }} onPress={() => { props._HandleOnPressItem(data, tipo, codigo, qtd, vlrPreco, corretora, vlrCorregatem, vlrTaxas, vlrTotal, vlrCustos, tipoInvest) }} >
          ...
          <View style={{ flex: 3, alignItems: 'center', justifyContent: 'center', }}>
            <Text style={[{ fontSize: 16, fontWeight: 'bold',  color: 'gray' }, ( tipo.substring(0,1) == 'C') && {color:'green'}, ( tipo.substring(0,1) == 'V') && {color:'red'}]}>R$ {vlrTotal}</Text>
          </View>
          <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', }}>
...
function OperacoesModalFiltro (props) {
  return(
    <Modal
    ...
    >  
      <View style={{ padding: 22, borderTopStartRadius: 20, borderTopEndRadius: 20, borderColor: 'rgba(0, 0, 0, 0.1)', backgroundColor: '#fff', }} >
        ...
        <Text style={{ fontSize: 12, fontWeight: 'bold', color: '#666666', marginBottom: 4, }}>Período:</Text>
...
const mapStateToProps = state => ({
  lstFiltroAtivos: state.operacoes.lstFiltroAtivos,
  txtFiltroMsgErro: state.operacoes.txtFiltroMsgErro,
  lstOperacoes: state.operacoes.lstOperacoes,
  dataProvider: state.operacoes.dataProvider,  
  isLoadingOperacoes: state.operacoes.isLoadingOperacoes,
})

const mapDispatchToProps = { limpaOperacoes, modificaMsgOperacoes, buscaListaAtivosOperacoes, buscaListaOperacoes, }
